...
| `.aidd/design-perspectives.md` | `/aidd:design-review` のプロジェクト固有観点 (任意)。`cp templates/design-perspectives.md.template .aidd/design-perspectives.md` で ASVS ベースのセキュリティ + 可観測性チェックリストから始められる |
...
| スクリプト | 動作 |
|---------|------|
| `session-start.sh` | SessionStart で aidd 資産の使いどころと「実装を左右する不明点は AskUserQuestion で確認」を注入。superpowers 未導入を検知して警告。20セッションごとに `/aidd:retro` を提案 (状態は `~/.claude/aidd/state.json`) |
| `usage-log.sh` | UserPromptSubmit 毎に aidd コマンド使用数・最終利用時刻を `~/.claude/aidd/usage.json` に記録。`AIDD_PROMPT_LOG=1` 時のみプロンプト履歴も保存 (`/aidd:retro` が読む) |
...
| `~/.claude/aidd/usage.json` | `/aidd:*` コマンドの使用数・最終使用時刻。`AIDD_PROMPT_LOG=1` 時のみ全プロンプトの先頭120文字 (最新200件、owner のみ読める権限で保存) | `usage-log.sh` |
...
| `AIDD_PROMPT_LOG=1` | `usage-log.sh` に全プロンプトの先頭120文字を記録し、`/aidd:retro` の昇格候補分析を有効化 |
...
| `design-perspectives.md.template` | `.aidd/design-perspectives.md` の出発点。OWASP ASVS 5.0 抜粋のセキュリティ観点 + 可観測性観点 (design-review の Agent 4 が読む) |
