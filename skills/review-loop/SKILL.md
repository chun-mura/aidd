...
レビュー終了時、ユーザー承認により棄却した指摘をプロジェクトローカルの `.aidd/review-dismissed.md` に追記保存する。ファイルがない場合は、次の表ヘッダーを作成してから追記する。

| 対象ドキュメント | 指摘の要約 | 棄却理由 | 棄却日 |
| --- | --- | --- | --- |

各エントリには対象ドキュメントのパス、指摘の要約、棄却理由、棄却日 (`YYYY-MM-DD`) を記録する。棄却の判断主体は従来どおりユーザーであり、agent が単独で棄却・保存してはならない。
